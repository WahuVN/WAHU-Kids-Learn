...
LANE_DONE=YES
PLAYABLE_EVENT_ENGINE_P0=GREEN
...
- Fresh completed-event replay: event đã terminal-complete mở lại tạo session mới 0/3, không resume terminal session, không reward sớm; đóng ở q1 rồi mở lại resume đúng replay session mới.
- Concurrent fresh replay: hai cửa sổ cùng chơi lại sau terminal event bind cùng replay session mới; `started_count` không double, Garden reward cũ không nhân và terminal session cũ không bị resume.
- Corrupt-catalog pending retry: event catalog hỏng giữa q2 retry vẫn fallback lesson presentation, giữ `RetryPending` + exact q2; retry-correct vẫn assisted, learning giữ nguyên và không fake reward.
...
- Complete-vs-suspend race: một wrapper terminalize 3/3 trong lúc wrapper stale `SuspendForBreak`/Dispose; completion vẫn thành công, runtime row không bị revive, session/progress/reward giữ terminal/idempotent.
- Same-event reward repair: nếu event cũ complete nhưng Garden write lỗi, bấm chơi lại cùng event tự repair reward session cũ rồi tạo replay session mới 0/3; replay chưa chơi không bị reward nhầm.
- Repeated reward-repair fault: nếu reconciliation vẫn lỗi khi mở replay, event vẫn playable 0/3 và không fake reward; sau khi fault hết, reopen resume đúng replay session/q1 rồi repair session cũ, không tạo session thứ ba.
- Zero-attempt Garden eligibility: completed Math session có 0 durable attempt không còn được tính vào `CompletedMathSessions`/milestone; một Rescue hợp lệ sau đó vẫn chỉ là growth=1, completed-rewardable=1 và còn 2 phiên tới mốc 3.
- Durable-attempt reward guard: `GrantCompletedMathSession` không còn tin `attempts` caller để quyết định eligibility; transaction verify actual durable attempt count, nên session completed 0 câu vẫn 0 reward dù caller truyền số dương.
- Root completion evidence guard: `MathSessionCoordinator.Complete()` reject adaptive 0-attempt và targeted lesson chưa đủ selected target; session/open question/progress/reward vẫn nguyên để Suspend/Abort/resume, không tạo terminal history rỗng/nửa chừng.
...
- MathSessionPersistenceRuntimeSmoke current: **7864 PASS**; gồm real 402-bank breadth stress + write-failure/corruption repair + deterministic authored runtime IDs + concurrent pending-retry semantics + generated/open-ordinal self-heal + late-review transaction rollback + operational DB failure fail-safe.
