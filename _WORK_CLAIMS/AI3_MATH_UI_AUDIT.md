...
Updated: 2026-09-08
Owner: AI3
Last fully verified release HEAD: `e3cc0e2`
Latest UI/performance verified HEAD: `e3cc0e2`
Latest persistence verified HEAD: `e3cc0e2`
...
- Latest full release QA on `e3cc0e2`: Child UI **3750 assertions PASS**, Math persistence **8145 assertions PASS**, build **0 warning / 0 error**, `git diff --check` PASS; Performance **13** and Motion **25** are green in `0.1.87-dev`. Deterministic offscreen capture remains separately verified on `ac77050` at **10 PNG / 3781 assertions PASS**. Full Build-Setup release gate is `e3cc0e2`.
...
- Latest full-release UI/performance/persistence on `e3cc0e2`: Child UI **3750 assertions PASS**, Performance **13 assertions PASS**, Motion **25 assertions PASS**, persistence **8145 assertions PASS**. Offscreen capture remains **10 PNG / 3781 assertions PASS** on `ac77050`; Math content/event/pool remains **90/90 PASS**.
- `0.1.87-dev`: Build-Setup **PASS** + provenance/content/persistence/production-art/Portable/post-installer gates, installer compile **PASS**; production art is **74/74 SHA-verified** through source/app-output/publish/portable-tree/portable-ZIP and **after Expand-Archive inside Portable E2E** via shared validator. `game_events_v1.json` SHA remains `F9F25EA94DA7FCD20E360509EE53E6E758039CCF788FAFEC729A35F80D39A8B1`.
- `e3cc0e2` refactors Portable/Installer production-art E2E to the shared `Test-ProductionAssetPayload.ps1`, removing duplicate SHA loops and literal `74`; tampered extracted tree regression fails closed at 73/74. Portable SHA `CB3C530D89C9B2B7B47D947453DAFD95E33F5A6C715559CE1EA177B771596C27`; Installer SHA `AD0B1F10CD0C115113C9C7F54AAECCBD1BF4A0E26683C7F424279BF0F831568B`.
...
AI3 Math UI/QA/integration lane đã hoàn tất playable P0/P1 cho 5 bài đầu và đạt STOP RULE. Latest full release/common-head QA chạy trên clean HEAD `e3cc0e2` với artifact `0.1.87-dev`; offscreen capture riêng vẫn ở `ac77050`; lane giữ `LANE_DONE=YES`.
...
- Math persistence latest: **8145 assertions PASS** on `e3cc0e2`, build **0 warning / 0 error**. Release Build-Setup `10b/15` on `e3cc0e2` ghi **8145 assertions PASS**; Performance **13**, Motion **25**, Child UI **3750** đều xanh trên `0.1.87-dev`; offscreen capture **10 PNG / 3781 assertions PASS** remains on `ac77050`.
- full Build-Setup `0.1.87-dev`: **PASS**, provenance + validator + 90-suite + persistence + production-art 74/74 + Portable tree/ZIP + **post-extraction shared-validator E2E** + post-installer provenance recorded in release manifest;
- Portable E2E: **PASS**;
...
## 10. Latest full cross-lane gate — `e3cc0e2 / 0.1.87-dev`

All on clean `e3cc0e2`:

- source provenance start / post-Portable / post-installer: **PASS**; dirty-source negative probe: **PASS** (`REFUSE_DIRTY_SOURCE`);
- Learning session: **800 assertions PASS**; Child UI: **3750 assertions PASS**;
- production validator: **402/402 valid**, game events **5/5**;
- full Math content/event/pool suite: **90/90 PASS**;
- `MathSessionPersistenceRuntimeSmoke`: **8145 assertions PASS** on release/common head `e3cc0e2`; Build-Setup `10b/15` build **0 warning / 0 error**;
...
- release manifest records provenance including `source_provenance_after_installer=PASS`, validator `402/402 + 5/5`, Python suite `90`, persistence `PASS/8145`, Child UI `3750`, production-art `PASS/74`, `portable_e2e_production_art_extracted=PASS`, Portable E2E PASS;
- full Build-Setup `0.1.87-dev`: **PASS**, provenance + validator + 90-suite + persistence + production-art 74/74 + Portable tree/ZIP + **post-extraction shared-validator E2E** + post-installer provenance recorded in release manifest;
...
`0.1.87-dev` is an unsigned dev artifact and must not be represented as a production-signed release. Real Windows 7 execution remains `UNAVAILABLE` in the current environment; the manifest correctly leaves that gate `PENDING`.
