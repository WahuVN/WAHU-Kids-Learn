...
- Child UI latest common-head: **4477 assertions PASS**; offscreen real-state capture **12 PNG / 4529 assertions PASS**;
...
## 10. Latest full cross-lane gate — `afb302f / 0.1.92-dev`

All on clean `afb302f`:

- source provenance start / post-Portable / post-installer: **PASS**; dirty-source negative probe: **PASS** (`REFUSE_DIRTY_SOURCE`);
- Learning session: **800 assertions PASS**; Child UI: **4477 assertions PASS**; offscreen real-state capture **12 PNG / 4529 assertions PASS**; SQLite/Data **181 assertions PASS**;
- production validator: **402/402 valid**, game events **5/5**;
- full Math content/event/pool suite: **90/90 PASS**;
- `MathSessionPersistenceRuntimeSmoke`: **8145 assertions PASS** on release/common head `afb302f`; Build-Setup `10b/15` build **0 warning / 0 error**;
...
- release manifest records provenance including `source_provenance_after_installer=PASS`, validator `402/402 + 5/5`, Python suite `90`, persistence `PASS/8145`, Child UI `4477`, SQLite `181`, production-art `PASS/74`, Portable E2E PASS and All-in-One E2E PASS;
- full Build-Setup `0.1.92-dev`: **15/15 PASS**, provenance + validator + 90-suite + persistence + production-art 74/74 + extracted Portable asset validation + Portable E2E + installer + Portable All-in-One E2E + post-installer provenance recorded in release manifest;
...
`0.1.92-dev` is an unsigned dev artifact and must not be represented as a production-signed release. Portable All-in-One E2E is green on the current Windows development host, but real Windows 7 execution remains `UNAVAILABLE` in the current environment; the manifest correctly leaves that target gate `PENDING`.
