...
- 267/267 distractor có **rationale unique / max repeat 1**, mỗi rationale sai chứa lời giải riêng của chính câu; 3 họ mẫu shallow cũ đã giảm **267 → 0**; **24 distractor thuật ngữ phép tính** có định nghĩa choice-specific bắt buộc; thêm **24 distractor cấu trúc số** (`dạng khai triển`, `so sánh`, `sắp xếp`) được validator tự tái tính lỗi từ prompt+choice, làm số câu MC/TF dùng chung thân rationale giảm **88 → 72** mà không dùng heuristic suy đoán;
