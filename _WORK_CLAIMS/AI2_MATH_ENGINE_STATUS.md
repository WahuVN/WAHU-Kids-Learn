...
- Concurrent completion convergence (`36bf7e5`): hai coordinator targeted/adaptive cùng terminalize một durable session đều hội tụ success nếu DB chứng minh đúng session đã `completed`; reward/mastery/progress vẫn exactly-once. Session `aborted` không bao giờ được nhận nhầm thành success.
- Open-question timestamp integrity (`91b3669`): `question_started_at_utc` malformed/missing hoặc nằm sau runtime checkpoint không còn silently biến thành `DateTime.UtcNow` rồi tạo `response_ms`/rapid-wrong giả. Engine bỏ riêng derived open-question cache, rewind về durable completed ordinal và regenerate đúng semantic question trong cùng session; targeted/adaptive đều có regression. `session.started_at_utc` malformed vẫn quarantine fail-closed, không mở rộng self-heal sang core session metadata.
- Core runtime timestamp ordering (`464355b`): `session.started_at_utc > math_session_runtime.updated_at_utc` dù parse hợp lệ vẫn là deterministic corruption; session cũ bị `recovered`, runtime checkpoint cũ bị xóa, không bị nhầm thành open-question-only self-heal.
- Targeted completion transaction fault (`3bdb205`): injected failure ở `math_lesson_progress.completed_count` sau 3 câu durable rollback cả terminal `session` update; coordinator vẫn active/resumable, reward=0. Gỡ fault rồi `Complete()` lại commit session/progress/reward exactly-once.
- Terminal loser convergence (`658a33a`): stale `Complete()` sau Abort và stale `Abort()` sau Complete vẫn rethrow `InvalidOperationException` để không che semantic conflict, nhưng coordinator đọc durable active-state và tự hội tụ `IsActive=false`. Thread race `Complete↔Abort` chỉ có một terminal winner; progress/reward khớp winner, attempts/mastery giữ exactly-once.
- Aborted runtime cleanup (`79a539d`): injected DELETE fault sau `Abort()` để lại đúng một derived runtime checkpoint nhưng session vẫn `aborted`, durable attempt/mastery giữ nguyên và reward=0. Start kế tiếp xóa stale checkpoint, tạo fresh session mới, không resume session aborted, `started_count` tăng đúng một lần.
...
- Expanded-bank integration: `373d9d1` bỏ test coupling `Single(...)`/`_01`, chạy được cả baseline 201 và pool 402; current real 402-bank persistence tổng **8044 assertions PASS**.
...
- Terminal-runtime cleanup: `MathSessionRuntimeService.DeleteTerminalCheckpoints(childId)` removes only derived runtime rows whose Math session is already terminal. `MathSessionCoordinator.Start` invokes it best-effort before resume lookup. Injected DELETE failure after completed **hoặc aborted** terminal state leaves durable learning intact and one repairable runtime row; next start cleans old row and keeps the new active runtime. Completed path preserves reward; aborted path preserves reward=0.
...
- Concurrent terminal completion: hai wrapper đã cùng replay đủ 3 checkpoint được barrier-thả `Complete()` song song; ít nhất một completion thành công, session cuối `completed`, lesson `completed_count=1`, attempts/mastery giữ 3/3 và `garden_growth` source_ref duy nhất = 1.
- Complete-vs-suspend race: sau 3/3, barrier thả một wrapper `Complete()` và wrapper stale `SuspendForBreak()` cùng lúc; không surface error, session cuối `completed`, runtime checkpoint=0, attempts/mastery=3/3, reward=1; Dispose stale sau đó vẫn không revive checkpoint/reward.
- Same-event reward repair: injected reward failure terminalizes event cũ với reward=0; sau khi bỏ fault, Start cùng event tạo session replay mới 0/3, repair đúng reward source_ref cũ=1, source_ref replay=0, progress `started=2/completed=1`, Garden progress chỉ tính completed session cũ.
- Reward-fault + stale Abort (`32a200d`): trước durable Complete có wrapper stale đã restore 3/3; reward insert bị fault nên completion vẫn terminal `completed` nhưng growth=0. Stale Abort sau đó reject + tự hội tụ inactive, không đổi winner hay tạo reward giả; replay kế tiếp repair đúng reward của completed session cũ.
- Recovered-terminal stale ops (`99b3d69`): session active bị mất runtime checkpoint rồi `RecoverDanglingSessions(child)` chuyển thành `recovered`; hai coordinator stale sau 3 durable questions gọi Complete/Abort đều reject, cùng hội tụ inactive, không tăng `completed_count`, không Garden reward, và vẫn giữ nguyên 3 attempts/3 mastery.
- Repeated reward-repair fault: giữ trigger reward failure cả lúc mở replay; `Start()` vẫn thành công với replay session 0/3, old/new reward đều 0 và progress `started=2/completed=1`. Sau khi bỏ trigger, reopen bằng lesson fallback resume đúng replay session + exact q1, repair old reward=1, replay reward=0 và không tăng started_count lần ba.
...
- Impossible lesson-progress fail-closed (`bf143a5`): access/prerequisite không còn tin row bất khả thi `completed_count>started_count`; effective completion trả 0 và không mở khóa bài kế. Raw history không bị mutate chỉ vì đọc, trong khi compatibility `child_skill.learning_state='STABLE'` vẫn được giữ nguyên.
- Real-start progress repair (`098f20e`): atomic targeted `TryCreateSession` và `MathLessonProgressStore.MarkStarted` tự reset completion-derived fields nếu durable row có `completed_count>started_count` trước khi tăng lần Start mới. Regression `1/2 + best=100` sau Start thành `2/0` với score/completion timestamp sạch; completion thật sau đó thành `2/1` và best score lấy từ phiên thật, không kế thừa 100% giả.
...
- Playable Event P0 persistence total trên current HEAD: **8044 assertions PASS**. Production AI1 catalog `bd1809e` đã tích hợp thật 5/5 event; synthetic fixtures vẫn giữ để fault/corruption/race test độc lập.
...
- MathSessionPersistenceRuntimeSmoke: **8044 PASS** — current full persistence/event/corruption/concurrency gate.
- AI1 working pool 402 integration: covered trong current **8044 PASS** persistence gate.
