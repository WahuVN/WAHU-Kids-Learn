...
LATEST_OFFSCREEN_CAPTURE_HEAD=`b2a6c9f`
...
- `468dd55` landed the visual polish; `af48f89` then bound game-art motion to the existing performance policy. Latest offscreen visual-capture evidence remains `b2a6c9f`; latest full release evidence is `330bb15`.
...
- Verification on `b2a6c9f`: Child UI **3743 assertions PASS**, offscreen capture **10 PNG / 3774 assertions PASS**, Performance **13 assertions PASS**, Motion **25 assertions PASS**, Math persistence **8077 assertions PASS**, `git diff --check` PASS.
