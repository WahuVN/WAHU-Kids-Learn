...
LAST_VERIFIED_RELEASE=`0.1.86-dev`_POST_INSTALLER_PROVENANCE_CONTENT_90_PERSISTENCE_8145_UI_3750_ASSET_74_PORTABLE_E2E_INSTALLER_PASS
...
- **Current release authority `0b3ab28 / 0.1.86-dev`**: Child UI **3750**, persistence **8145**, validator **402/402 + 5/5**, content/event/pool **90/90**, production assets **74/74**, Portable E2E PASS, installer compile PASS, provenance start/final/post-installer PASS.
- Packaging chronology is fail-closed: `0.1.84-dev` rejected for missing production art in publish/ZIP; `0.1.85-dev` rejected when the first ZIP validator failed on Windows path separators; corrected `0b3ab28` validates the real `0.1.86-dev` ZIP 74/74.
- Completion chain now includes `68f94a1` durable mastery outcomes, `298a764` semantic commit keys, and `5d8555d` exact deterministic selected-question IDs. Portable SHA `9AFD93A1E974C8C9BD564FCCE020A78D406151CBB4884489E9DBA2D18F77891F`; Installer SHA `6DB9D52C97CA5F6F1ED45A4473E33A4DED41BAE460929713C69A53B3785E0EE7`. Older release notes below are historical.
- `468dd55` landed the visual polish; `af48f89` then bound game-art motion to the existing performance policy. Latest offscreen-capture QA evidence remains `3ec4b86`; latest full release evidence is `0b3ab28`.
...
- Release-full evidence is now `0b3ab28` / `0.1.86-dev`: full Build-Setup/Installer distribution gate trên cùng clean common head ghi Child UI **3750**, persistence **8145**, production assets **74/74**, Portable E2E PASS và post-installer provenance PASS.
...
Current closure evidence on `0b3ab28`: source provenance **PASS** cả sau installer, Learning **800**, Child UI **3750**, persistence **8145**, content/event **90/90**, validator **402/402 + 5/5**, production assets **74/74**, Build-Setup `0.1.86-dev` **PASS** với content/persistence/asset/Portable/post-installer provenance là first-class gates, installer compile **PASS**. Full Installer E2E is externally safety-blocked by unowned learner data; production signing và real Windows 7 validation vẫn PENDING/UNAVAILABLE.
