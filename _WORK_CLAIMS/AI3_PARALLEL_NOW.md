...
LAST_VERIFIED_RELEASE=`0.1.69-dev`_POST_INSTALLER_PROVENANCE_CONTENT_90_PERSISTENCE_8044_PORTABLE_E2E_INSTALLER_PASS
...
- `468dd55` landed the visual polish; `af48f89` then bound game-art motion to the existing performance policy. Current release/common-head verification is `d821874`.
...
- Verification on release head `d821874`: Child UI **3727 assertions PASS**, Performance **13 assertions PASS**, Motion **25 assertions PASS**, Math persistence **8044 assertions PASS**, validator **402/402 + 5/5**, content/event/pool **90/90 PASS**, `git diff --check` PASS.
- Release-full `0.1.69-dev` on `d821874`: Build-Setup **15/15 PASS**, Portable E2E **PASS**, installer compile **PASS**, source provenance before/after installer **PASS**. Full Installer E2E remains **BLOCKED_SAFETY** by unowned learner data; production signing and real Windows 7 validation remain external blockers.
...
Current closure evidence on `d821874`: source provenance **PASS** cả sau installer, Learning **800**, Child UI **3727**, persistence **8044**, content/event **90/90**, validator **402/402 + 5/5**, Build-Setup `0.1.69-dev` **15/15 PASS** với content/persistence/Portable/post-installer provenance là first-class gates, Portable E2E **PASS**, installer compile **PASS**. Full Installer E2E remains externally safety-blocked by unowned learner data; production signing and real Windows 7 validation remain unavailable.
