...
LATEST_UI_VERIFIED_HEAD=`3299b66`
LATEST_PERSISTENCE_VERIFIED_HEAD=`3299b66`
MATH_3AI_DONE=YES
AI3_LANE_DONE=YES
...
- **Visual/motion integration common-head `3299b66`: GREEN** — Home/Hub/lesson/rescue có visual polish mới; `af48f89` nối game-art motion vào performance contract để LOW tĩnh, NORMAL có FPS cap và learning-focus feedback tĩnh; `a0344f4` theo dõi hierarchy cha để container ẩn/offscreen dừng timer con, và `dcf844b` khóa detach/re-attach để control rời visual tree cũng dừng timer rồi resume an toàn khi gắn lại. Không đổi grading/session semantics. Child UI **3727 assertions PASS**, Performance **13 assertions PASS**, Motion **25 assertions PASS**, persistence **8039 assertions PASS**, `git diff --check` PASS.
- Release-full artifact vẫn là `0.1.68-dev` tại `1c4ce9e`; `3299b66` là common-head UI/performance/persistence verification mới nhất. Chưa gọi đây là release artifact mới cho tới khi full Build-Setup/Installer gate chạy lại.
...
- Latest UI/performance/persistence: Child UI **3727 assertions PASS**, Performance **13 assertions PASS**, Motion **25 assertions PASS**, persistence **8039 assertions PASS** on `3299b66`. Most recent full Math content/event/pool suite remains **90/90 PASS**; visual/motion integration did not change authored content.
...
- **Engine: 100% theo Math lane** — schema V5 + pack identity, targeted session 3 câu từ pool 6, durable ordered selected-set, exact resume/retry/corrupt recovery, rescue runtime/checkpoint/repair/terminal/reward idempotency. Latest `MathSessionPersistenceRuntimeSmoke`: **8039 assertions PASS** on `3299b66`.
- **UI/QA: playable P0 GREEN cho 5 bài đầu** — Home/Hub rescue entry, 5 mission cards, 3 checkpoint cards, behavior-aware repair/break, exact rescue resume, fail-safe event fallback, Garden completion, accessibility/keyboard/responsive, plus post-closure visual polish. Latest `ChildUiRuntimeSmoke`: **3727 assertions PASS** on `3299b66`; Performance **13 assertions PASS**; Motion **25 assertions PASS**.
