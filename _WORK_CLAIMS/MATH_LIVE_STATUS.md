...
- **Visual/motion integration release/common-head `d821874`: GREEN** — Home/Hub/lesson/rescue có visual polish mới; `af48f89` nối game-art motion vào performance contract để LOW tĩnh, NORMAL có FPS cap và learning-focus feedback tĩnh; `a0344f4` theo dõi hierarchy cha để container ẩn/offscreen dừng timer con, và `dcf844b` khóa detach/re-attach để control rời visual tree cũng dừng timer rồi resume an toàn khi gắn lại. Không đổi grading/session semantics. Child UI **3727 assertions PASS**, Performance **13 assertions PASS**, Motion **25 assertions PASS**, persistence **8044 assertions PASS**, `git diff --check` PASS.
- Release-full artifact mới là `0.1.69-dev` tại `d821874`: Build-Setup **15/15 PASS**, Portable E2E **PASS**, installer compile **PASS**, source provenance trước/sau installer **PASS**. Full Installer E2E vẫn **BLOCKED_SAFETY** vì learner data không có ownership marker.
...
- Latest release/common-head UI/performance/persistence on `d821874`: Child UI **3727 assertions PASS**, Performance **13 assertions PASS**, Motion **25 assertions PASS**, persistence **8044 assertions PASS**. Most recent full Math content/event/pool suite remains **90/90 PASS**; visual/motion integration did not change authored content.
- `0.1.69-dev`: Build-Setup **15/15 PASS** + provenance/content/persistence/Portable first-class gates + post-installer provenance, Portable E2E **PASS**, installer compile **PASS**.
- Portable SHA256 `748E01A04883C64E79AE0297CEA21FB7F02C5F822D991E2908E1E6F0822FA494`; Installer SHA256 `6C3D0FAE1B65053C833743CEFBE36AB655884D466DD1A13BCF95870630184F63`.
...
## Latest fully verified gates — `d821874` / `0.1.69-dev`

- Source provenance đầu build, sau Portable và **sau installer/update-manifest**: **PASS**; manifest ghi `source_provenance/source_tree_clean/source_commit_stable/source_provenance_after_installer=PASS` và bind exact Git `d8218744b9d6e1a23edc8ad260a58f25468f75fc`.
...
- Learning session vertical-slice: **800 assertions PASS**; Child UI: **3727 assertions PASS**.
...
- Persistence: Build-Setup `10b/15` build **0 warnings / 0 errors**, **8044 assertions PASS**.
...
- Build-Setup `0.1.69-dev`: **15/15 PASS** cùng sub-gates `0/15`, `6b`, `6c`, `10b`, `13b`, `13c`, `15b`.
...
- Artifact `0.1.69-dev`: Portable `748E01A04883C64E79AE0297CEA21FB7F02C5F822D991E2908E1E6F0822FA494`; Installer `6C3D0FAE1B65053C833743CEFBE36AB655884D466DD1A13BCF95870630184F63`.
...
1. production Authenticode signing — artifact `0.1.69-dev` hiện là unsigned dev build (`signed=false`);
