...
Updated: 2026-09-08
LAST_FULLY_VERIFIED_HEAD=`330bb15`
LATEST_UI_VERIFIED_HEAD=`330bb15`
LATEST_PERSISTENCE_VERIFIED_HEAD=`330bb15`
MATH_3AI_DONE=YES
AI3_LANE_DONE=YES
...
- **Visual/motion QA head `b2a6c9f`: GREEN** — Home/Hub/lesson/rescue có visual polish mới; `af48f89` nối game-art motion vào performance contract để LOW tĩnh, NORMAL có FPS cap và learning-focus feedback tĩnh; `a0344f4` theo dõi hierarchy cha để container ẩn/offscreen dừng timer con, và `dcf844b` khóa detach/re-attach để control rời visual tree cũng dừng timer rồi resume an toàn khi gắn lại. `cd0cc91` thêm deterministic offscreen capture không cần desktop interactive. Không đổi grading/session semantics. Child UI **3743 assertions PASS**, offscreen capture **10 PNG / 3774 assertions PASS**, Performance **13 assertions PASS**, Motion **25 assertions PASS**, persistence **8077 assertions PASS**, `git diff --check` PASS.
- Release-full artifact hiện tại là `0.1.83-dev` tại clean `330bb15`; full Build-Setup/Installer distribution gate, Portable E2E và post-installer provenance đều PASS trên cùng source head.
...
- Latest release QA on `330bb15`: Child UI **3743 assertions PASS**, Performance **13 assertions PASS**, Motion **25 assertions PASS**, persistence **8124 assertions PASS**, full Math content/event/pool **90/90 PASS**. Latest deterministic offscreen capture remains **10 PNG / 3774 assertions PASS** on `b2a6c9f` because the later persistence hardening did not change visual code.
...
- **Engine: 100% theo Math lane** — schema V5 + pack identity, targeted session 3 câu từ pool 6, durable ordered selected-set, exact resume/retry/corrupt recovery, rescue runtime/checkpoint/repair/terminal/reward idempotency. Latest `MathSessionPersistenceRuntimeSmoke`: **8124 assertions PASS** on `330bb15`.
- **UI/QA: playable P0 GREEN cho 5 bài đầu** — Home/Hub rescue entry, 5 mission cards, 3 checkpoint cards, behavior-aware repair/break, exact rescue resume, fail-safe event fallback, Garden completion, accessibility/keyboard/responsive, plus post-closure visual polish. Latest `ChildUiRuntimeSmoke`: **3743 assertions PASS** on `330bb15`; Performance **13 assertions PASS**; Motion **25 assertions PASS**. Latest offscreen capture remains **10 PNG / 3774 assertions PASS** on `b2a6c9f`.
...
## Latest fully verified gates — `330bb15`
...
- Learning session vertical-slice: **800 assertions PASS**; Child UI: **3743 assertions PASS**.
...
- Persistence: Build-Setup `10b/15` build **0 warnings / 0 errors**, **8124 assertions PASS**.
...
- Build-Setup `0.1.83-dev`: **15/15 PASS** cùng sub-gates `0/15`, `6b`, `6c`, `10b`, `13b`, `13c`, `15b`.
...
- Artifact `0.1.83-dev`: Portable `40EDEB9F2C988FF1A5216C5BB24D5B3BC30F4EC1B2FAE0E1F4C1FF3A4E4D51AE`; Installer `ADA17E788EEECBDD48C2169B7FE4B95F0ABF59D49A4CF71A537D6892BC4C016E`.
