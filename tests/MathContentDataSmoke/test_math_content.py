...
class MathContentDataSmoke(unittest.TestCase):
    ...
    def test_expanded_component_term_distractors_have_structured_diagnoses(self):
        target_skills = {
            ...
            "MULTIPLICATION_COMPONENTS", "DIVISION_COMPONENTS",
        }
        checked = 0
        for item in self.questions:
            if item["skill_id"] not in target_skills or int(item["id"][-2:]) < 4:
    ...
    def test_expanded_shape_reasoning_distractors_have_structured_diagnoses(self):
        target_skills = {"SPHERE_RECOGNIZE", "THREE_COLLINEAR_POINTS", "FOLD_CUT_COMPOSE_SHAPES"}
        checked = 0
        for item in self.questions:
            if item["skill_id"] not in target_skills or int(item["id"][-2:]) < 4:
                continue
    ...
    def test_expanded_measurement_time_mass_distractors_have_structured_diagnoses(self):
        target_skills = {"HEAVIER_LIGHTER", "CLOCK_MINUTE_HAND_AT_3_OR_6", "MEASUREMENT_ESTIMATE_BASIC"}
        checked = 0
        for item in self.questions:
            if item["skill_id"] not in target_skills or int(item["id"][-2:]) < 4:
                continue
    ...
    def test_expanded_geometry_distractors_have_structured_diagnoses(self):
        target_skills = {"POINT_RECOGNIZE", "LINE_SEGMENT_RECOGNIZE", "POLYLINE_RECOGNIZE"}
        checked = 0
        for item in self.questions:
            if item["skill_id"] not in target_skills or int(item["id"][-2:]) < 4:
                continue
