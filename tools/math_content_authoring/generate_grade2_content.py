...
def distractor_rationale(choice_text: str, correct_text: str, explanation: str, skill: str, prompt: str) -> str:
    ...
    if structured_reason:
        if term_reason:
            return f"“{choice_text}” chưa đúng. {structured_reason} {term_reason} {reason}"
...
LESSON_INFO = {
    ...
    "NUM_FULL_HUNDREDS_RECOGNIZE": ("m2_tp01_count_place", "Nhận biết số tròn trăm", "Số tròn trăm có hàng chục và hàng đơn vị đều bằng 0, ví dụ 300, 700, 1000. Có thể xem đó là một số lượng gồm các nhóm 100 đầy đủ.", "Số tròn trăm", "Số tròn trăm có hàng chục và hàng đơn vị đều bằng 0; vì vậy hai chữ số cuối của số đều là 0.", ["NUM_COUNT_READ_WRITE_0_1000"]),
    "NUM_PREDECESSOR_SUCCESSOR": ("m2_tp01_count_place", "Số liền trước và số liền sau", "Số liền trước của một số nhỏ hơn số đó đúng 1; số liền sau lớn hơn đúng 1. Quy tắc này dùng được cả khi chuyển qua chục hoặc trăm.", "Liền trước và liền sau", "Số liền trước được tìm bằng cách bớt 1; số liền sau được tìm bằng cách thêm 1, miễn là số vẫn trong phạm vi đã học.", ["NUM_COUNT_READ_WRITE_0_1000"]),
